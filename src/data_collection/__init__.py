...
CASSANDRA_HOST = config.get("cassandra", "host", fallback="cassandra")
...
STOCK_SYMBOLS = config.get(
    "market_data", "symbols", fallback="AAPL,MSFT,GOOGL,AMZN,TSLA"
).split(",")
COLLECTION_INTERVAL = config.getint(
    "general", "collection_interval_seconds", fallback=60
)
TWITTER_SEARCH_TERMS = config.get(
    "twitter", "search_terms", fallback="stock market,investing"
).split(",")
REDDIT_SUBREDDITS = config.get(
    "reddit", "subreddits", fallback="stocks,investing,wallstreetbets"
).split(",")
NEWS_SOURCES = config.get(
    "news_api", "sources", fallback="bloomberg,financial-times,business-insider"
)
