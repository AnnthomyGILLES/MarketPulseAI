...
from src.data_collection import (
    ...
    STOCK_SYMBOLS, KAFKA_BOOTSTRAP_SERVERS
)
...
class StockDataProcessor:
    ...
    def __init__(self, kafka_brokers=KAFKA_BOOTSTRAP_SERVERS, kafka_topic=MARKET_DATA_TOPIC,
                 ...
                 cassandra_keyspace="market_data", cassandra_table="stock_features"):
    ...
    def validate_data(self, kafka_stream):
        ...
        parsed_stream = kafka_stream \
            .select(from_json(col("value").cast("string"), self.stock_schema).alias("data")) \
            .select("data.*")
    ...
    def compute_features(self, validated_stream):
        ...
        processed_stream = validated_stream \
            .withColumn("returns", (col("close") - lag("close", 1).over(Window.partitionBy("Name").orderBy("date"))) / lag("close", 1).over(Window.partitionBy("Name").orderBy("date"))) \
            ...
            .withColumn("upper_band", col("sma_20") + (2 * stddev("close").over(window_20))) \
            .withColumn("lower_band", col("sma_20") - (2 * stddev("close").over(window_20))) \
            ...
            .withColumn("rsi_diff", when(col("close") > lag("close", 1).over(prev_row), col("close") - lag("close", 1).over(prev_row)).otherwise(0)) \
            .withColumn("rsi_down", when(col("close") < lag("close", 1).over(prev_row), lag("close", 1).over(prev_row) - col("close")).otherwise(0))
    ...
    def write_to_cassandra(self, feature_stream):
        ...
        """
        logger.info(f"Writing data to Cassandra keyspace={self.cassandra_keyspace}, table={self.cassandra_table}")
