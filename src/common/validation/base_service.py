...
class BaseValidationService(abc.ABC):
    ...
    def __init__(
        ...
        self.config_path = Path(
            config_path or (self.DEFAULT_CONFIG_DIR / self.DEFAULT_CONFIG_FILE)
        )
        logger.info(
            f"[{self.service_name}] Loading Kafka configuration from: {self.config_path}"
        )
        try:
            self.config = load_config(str(self.config_path))
            # Basic validation of required keys directly in the loaded config
            if "topics" not in self.config:
                raise KeyError("Missing 'topics' key in config.")
            if "consumer_groups" not in self.config:
        ...
        self.stats_interval = self.config.get("config", {}).get(
    ...
    def _setup_logging(self):
        ...
        logger.add(
            ...
            rotation="100 MB",
            retention="10 days",
            level=self.config.get("logging", {}).get(
                "level", "INFO"
            ),  # Configurable level
        ...
        logger.info(
            f"[{self.service_name}] Logging configured. Log files in: {log_dir}"
        )
    ...
    def _report_stats(self, current_time: Optional[float] = None):
        ...
        if processed > 0:
            ...
            logger.info(
                f"[{self.service_name}] Stats ({int(current_time - self.stats['last_report_time'])}s): No messages processed yet."
            )
        ...
        try:
            ...
            if not isinstance(data, (dict, list, str, int, float, bool, type(None))):
                ...
                if hasattr(data, "model_dump"):
                    ...
                    logger.warning(
                        f"{log_prefix} Payload for {msg_type} topic is not easily serializable ({type(data)}), converting to string. Context: {context}"
                    )
                    payload = str(data)
            else:
                payload = data
            ...
            logger.debug(
                f"{log_prefix} Sending {msg_type} message | Topic: {topic}, Key: {key} | Context: {context}"
            )
            success = producer.send_message(topic=topic, value=payload, key=key)
            if success:
                logger.debug(
                    f"{log_prefix} Successfully queued {msg_type} message | Topic: {topic}, Key: {key} | Context: {context}"
                )
                return True
            else:
                logger.error(
                    f"{log_prefix} Failed to queue {msg_type} message (send_message returned False) | Topic: {topic}, Key: {key} | Context: {context}"
                )
                return False
        except Exception as e:
            logger.exception(
                f"{log_prefix} Exception sending {msg_type} message | Topic: {topic}, Key: {key}, Error: {e} | Context: {context}"
            )
            return False
    ...
    def process_message(self, raw_message: Dict[str, Any]) -> None:
        ...
        logger.debug(
            f"{log_prefix} Received message | Value: {str(message_value)[:200]}... | Context: {log_context}"
        )
        ...
        try:
            ...
            if is_valid and validated_data is not None:
                logger.debug(
                    f"{log_prefix} Validation SUCCEEDED | Context: {log_context}"
                )
                validated_key = (
                    self._get_validated_message_key(validated_data)
                    or f"valid_{item_id}"
                )
                ...
                self._update_and_report_stats(
                    is_valid=True, is_error=not send_success
                )  # Count as error if send fails
                    ...
                    for msg in message_generator:
                        ...
                        if msg is not None:
                            if "error" in msg and msg["error"] is not None:
                                ...
                                logger.warning(
                                    f"{log_prefix} Received unexpected message structure from consumer: {msg}"
                                )
    ...
    def _handle_signal(self, signum, frame):
        ...
        if self.running:
            logger.warning(
                f"[{self.service_name}] Received signal {signal_name}. Initiating graceful shutdown..."
            )
            ...
            logger.warning(
                f"[{self.service_name}] Received signal {signal_name}, but service already stopping."
            )
    ...
    def run_service(cls, *args, **kwargs):
        ...
        try:
            ...
            logger.exception(
                f"[{cls.__name__}] Failed to initialize or run service: {e}"
            )
